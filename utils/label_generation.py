...
def process_grasp_labels(end_points):
    ...
    seed_xyzs = end_points['grasp_points']  # (B, Ns, 3)
    ...
    label_mask = (batch_grasp_labels > 0) & (batch_grasp_widths <= GRASP_MAX_WIDTH)  # must smaller than max width
    u_max = batch_grasp_labels.max()
    batch_grasp_labels[label_mask] = torch.log(u_max / batch_grasp_labels[label_mask])  # why? expand range?
    batch_grasp_labels[~label_mask] = 0
    batch_grasp_view_scores, _ = batch_grasp_labels.view(batch_size, num_samples, V, A*D).max(dim=-1)  # (B, Ns, V)
...
def match_grasp_view_and_label(end_points):
    ...
    top_view_inds = end_points['grasp_top_view_inds']  # (B, Ns) network prediction rather than ground truth
    ...
    top_view_grasp_offsets = torch.gather(grasp_offsets, 2, top_view_inds_).squeeze(2)  # (B, Ns, A, D, 3)
    ...
    return top_template_views_rot, top_view_grasp_labels, top_view_grasp_offsets, end_points
