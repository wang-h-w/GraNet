...
from granet_pipeline import GraNet, get_loss
from pytorch_utils import BNMomentumScheduler
from graspnet_dataset_granet import GraspNetDataset, collate_fn, load_grasp_labels
...
parser.add_argument('--visualize', action='store_true', help='key point selection visualization')
...
if not os.path.exists(os.path.join(cfgs.log_dir, 'checkpoints')):
    os.mkdir(os.path.join(cfgs.log_dir, 'checkpoints'))
...
net = GraNet(batch_size=cfgs.batch_size, input_feature_dim=3, num_view=cfgs.num_view, num_angle=12, num_depth=4,
...
if cfgs.visualize:
    SELECT_POINT_WRITER = SummaryWriter(os.path.join(cfgs.log_dir, 'points'))
...
def train_one_epoch():
    ...
    net.train()
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):  # when data type is dict, load batch IN key!
        for key in batch_data_label:
            if 'list' in key:  # train mode
                for i in range(len(batch_data_label[key])):
                    for j in range(len(batch_data_label[key][i])):
                        batch_data_label[key][i][j] = batch_data_label[key][i][j].to(device)  # load on gpu
            elif 'graph' in key:
                for i in range(len(batch_data_label[key])):
                    batch_data_label[key][i] = batch_data_label[key][i].to(device)
            else:  # test mode
                batch_data_label[key] = batch_data_label[key].to(device)  # load on gpu
        ...
        if (batch_idx+1) % 100 == 0 and cfgs.visualize:

            pc0 = vis_sample(end_points['vis_origin'][0], end_points['vis0_idx'][0])
            SELECT_POINT_WRITER.add_3d("select_point_stage_0", to_dict_batch([pc0]),
                                       step=(EPOCH_CNT*len(TRAIN_DATALOADER)+batch_idx)*cfgs.batch_size)

            pc1 = vis_sample(end_points['vis0_xyz'][0], end_points['vis1_idx'][0])
            SELECT_POINT_WRITER.add_3d("select_point_stage_1", to_dict_batch([pc1]),
                                       step=(EPOCH_CNT * len(TRAIN_DATALOADER) + batch_idx) * cfgs.batch_size)

            pc2 = vis_sample(end_points['vis1_xyz'][0], end_points['vis2_idx'][0])
            SELECT_POINT_WRITER.add_3d("select_point_stage_2", to_dict_batch([pc2]),
                                       step=(EPOCH_CNT * len(TRAIN_DATALOADER) + batch_idx) * cfgs.batch_size)
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, cfgs.max_epoch):
        ...
        torch.save(save_dict, os.path.join(cfgs.log_dir, 'checkpoints', 'checkpoint'+str(epoch)+'.tar'))
