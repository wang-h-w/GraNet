...
class ApproachNet(nn.Module):
    def __init__(self, num_view, seed_feature_dim, neighbor_radius=0.02, nsample=32):
        ...
        self.neighbor_feat_dim = 0
        self.in_dim = seed_feature_dim
        self.radius = neighbor_radius
        self.nsample = nsample
        self.ball_query = BallQuery.apply
        self.conv1 = nn.Conv1d(self.in_dim + self.neighbor_feat_dim, self.in_dim, 1)
        ...
        self.bn1_1 = nn.BatchNorm1d(self.in_dim)
    ...
    def forward(self, seed_xyz, seed_features, end_points):
        ...
        input_feat = seed_features

        features = F.relu(self.bn1(self.conv1(input_feat)), inplace=True)
        ...
        vp_xyz_ = vp_xyz.view(-1, 3)  # (B*num_seed, 3)
        batch_angle = torch.zeros(vp_xyz_.size(0), dtype=vp_xyz.dtype, device=vp_xyz.device)  # (B*num_seed, )
        vp_rot = batch_viewpoint_params_to_matrix(-vp_xyz_, batch_angle).view(B, num_seed, 3, 3)  # rotation matrix
...
class OperationNet(nn.Module):
    ...
    def forward(self, vp_features, end_points):
        ...
        # split prediction
        end_points['grasp_score_pred'] = vp_features[:, 0:self.num_angle]  # (B, A, Ns, D)
        end_points['grasp_angle_cls_pred'] = vp_features[:, self.num_angle:2*self.num_angle]  # (B, A, Ns, D)
        end_points['grasp_width_pred'] = vp_features[:, 2*self.num_angle:3*self.num_angle]  # (B, A, Ns, D)
        return end_points
